  ...
  getComedianWithMentions().map(c => {
    return {
      ...
      subjects: c.subjects.map(s => {
        ...
        if (!subject) console.log("Subject: " + JSON.stringify({ id: s, name: s }))
        return subject.name
      }),
      genres: c.genres.map(s => {
        const genre = genres.find(x => x.id == s)
        if (!genre) console.log("Genre: " + JSON.stringify({ id: s, name: s }))
        return genre.name
      })
    }
  }).forEach(c => {
...
function getComedianFitness(source, target) {
  const sourcePropsLength = source.subjects.length + source.genres.length
  const targetPropsLength = target.subjects.length + target.genres.length

  if (sourcePropsLength + targetPropsLength == 0) {
    return 0
  }
  ...
  return (commonSubjectsLength + commonGenresLength) / Math.max(sourcePropsLength, targetPropsLength)
}