...
function enrichDatabase() {
  return episodes.map(e => {
    ...
    e.sections = e.sections.map(s => {
      s.section = sections.find(x => x.id == s.section).name;
    ...
    return flatMap(e.sections, s => {
      if (s.extra != undefined && s.extra.comedians != undefined) {
        return s.extra.comedians.map(c => {
          const comedian = comedians.find(x => x.id == c.id);
...
function writeSections() {
  ...
  fs.writeFileSync(`${BASE_PATH}/sections.json`, info)
}
